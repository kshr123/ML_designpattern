...
**目的**: 全体感の理解と新規技術の「なぜ使うのか」「どう使うのか」を明確にする

#### 1. ソースコード（Python, JavaScript等）

- **クラス・関数の役割**: docstringで必ず記載
- **複雑なロジック**: inline commentで意図を説明
- **TODO/FIXME**: 将来の改善点を明記
- **なぜそうするのか**: 実装の背景・理由を書く（特に一見わかりにくい実装）
...
def preprocess_image(image: Image) -> np.ndarray:
    ...
    なぜImageNet標準を使うのか:
        - ResNet50はImageNetで学習されているため
        - 同じ正規化パラメータを使わないと精度が低下する
...
**全体像の理解を重視**する。各セクションに以下を含める：

- **ファイル冒頭**: このDockerfileの目的、なぜこの構成なのか
- **各ステージ**: なぜマルチステージビルドを使うのか
- **主要コマンド**: なぜこのパッケージが必要なのか、何に使うのか
- **環境変数**: それぞれの環境変数の意味と用途
- **ポート**: どのサービスがどのポートを使うのか
...
#
# 【マルチステージビルドを使う理由】
#   - Stage 1: ビルドに必要な依存をインストール
#   - Stage 2: ランタイムに必要なファイルのみコピー
#   - 結果: 最終イメージが軽量化（XXX MB削減）
#
# ================================================================================

# Stage 1: ビルドステージ
# 【目的】重い依存関係でビルド、成果物を生成
FROM python:3.13-slim AS builder

# なぜuvを使うのか: pipより10-100倍高速
...
**サービス間の関係と全体のデータフロー**を明確にする：

- **ファイル冒頭**: システム全体像（ASCII図推奨）、データフロー
- **各サービス**: 役割、なぜ分離するのか、依存関係の理由
- **環境変数**: 設定値の意味と他サービスへの影響
- **ネットワーク**: サービス間通信の仕組み

```yaml
# ================================================================================
# docker-compose.yml - パターン名 マルチコンテナ構成
# ================================================================================
#
# 【システム全体像】
#   ┌─────────────┐
...
#
# 【データフロー】
#   1. Client → Service A (リクエスト受付)
#   2. Service A → Service B (前処理)
#   3. Service B → Service C (推論)
#   ...
#
# ================================================================================
...
**学習者・新規参加者が理解しやすい**構成にする：

必須セクション：
- **概要**: このパターンは何を解決するのか
- **アーキテクチャ図**: 視覚的な全体像
- **なぜこの技術を使うのか**: 技術選定の理由
- **どう使うのか**: セットアップ手順、実行方法
- **学習ポイント**: 重要な概念、ハマりどころ
...
### なぜこの技術を使うのか
- **gRPC**: HTTP/2ベースで高速、バイナリ通信
- **Redis**: インメモリで高速、キューのサポート
...
実装完了後、以下のタイミングでコメント拡充を実施：

1. **実装直後**: ロジックが頭に残っているうちに
2. **リファクタリング時**: コード整理と同時にコメントも整理
3. **レビュー前**: 他者（未来の自分含む）が読みやすいように
4. **パターン完成時**: Dockerfile、docker-compose.yml、README.mdを総仕上げ
